...
| R12 | Duffel Flights/Stays payment architecture undecided -- changes PCI scope and possibly Seller-of-Travel exposure | M | H | Researched 2026-08-22 (Duffel docs): only the Payment Intent -> Balance model lets Drift keep a markup through Duffel's own infra -- plain Card passthrough forbids markup entirely, plain pre-funded Balance requires Drift to run its own separate Stripe-style processor. Building checkout around Payment Intent -> Balance; Duffel's Balance payout mechanics (minimum amount, timing, any withdrawal fee) are NOT yet confirmed -- their payout help article 403'd on fetch, needs a direct question to Duffel before launch | Andre | Open -- building |
