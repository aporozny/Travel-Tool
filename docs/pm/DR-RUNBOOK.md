...
- Travelport (2026-09-08, updated): sandbox creds at my.travelport.com. **Working sandbox connection confirmed for BOTH Air and Hotel** using header `TVP-PCC-Core: 599B_1G` (no XAUTH_TRAVELPORT_ACCESSGROUP needed -- that header, which docs kept pointing to, turned out to be a red herring for this flow; sending a *guessed* value for it actively broke an otherwise-working request). `599B_1G` is almost certainly a shared/public sandbox PCC, not a dedicated one -- fine for proving the integration, NOT for real production bookings. Auth: OAuth2 password grant, `https://auth.pp.travelport.com/oauth/token`, JSON body `{client_id, client_secret, username, password, grant_type:"password"}`, Bearer token valid 24h. API host: `https://api.pp.travelport.net` (note .net for API, but .com for auth -- confirmed by testing both, docs are inconsistent). Air endpoint verified live: `POST /11/air/catalog/search/catalogproductofferings`. Hotel endpoint verified live: `POST /11/hotel/search/properties/search`. TRAVELPORT_PASSWORD contains shell-special characters (braces, an en-dash) -- always single-quote it in .env or `source` will break. Still requesting a real, dedicated PCC from Travelport support for production use.
